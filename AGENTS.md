...
- Plugin boot cache (`modules/cache/cache.ps1`): TTL-based, skips MD5 hash on hot path (~5ms)

## Conventions

- `Set-StrictMode -Version Latest` everywhere — no uninitialized vars, no hidden scoping
- `$script:` scope for module-private variables, `$global:` only for `__ProfileRepoRoot` and `ProfileLoaded`
