...
```powershell
# CI runs these 2 files (both use custom framework):
...
CI (`.github/workflows/test.yml`) copies profile + modules to `$PROFILE` path, runs both on push/PR to `main`.
...
.\install.cmd                    # double-click (Windows)
